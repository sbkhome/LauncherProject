...
import com.android.medianet.launcher.middlelayer.LoaderTask
...
class LauncherModel(
    ...
    private val mAppAppsList = AllAppsList(iconCache)
    ...
    private fun startLoader(newCallbacks: Array<BgDataModel.Callbacks>): Boolean {
        ...
        synchronized(mLock) {
            ...
            if (callbacksList.isNotEmpty()) {
                ...
                val launcherBinder =
                    BaseLauncherBinder(mApp, mBgDataModel, mAppAppsList, callbacksList)
                if (bindDirectly) {
    ...
    fun enqueueModelUpdateTask(task: ModelUpdateTask) {
        ...
        MODEL_EXECUTOR.execute {
            ...
            task.execute(
                ModelTaskController(mApp, mBgDataModel, mAppAppsList, this, MAIN_EXECUTOR),
                mBgDataModel,
                mAppAppsList,
            )
        }
    ...
    fun dumpState(prefix: String?, fd: FileDescriptor?, writer: PrintWriter, args: Array<String?>) {
        if (args.isNotEmpty() && TextUtils.equals(args[0], "--all")) {
            writer.println(prefix + "All apps list: size=" + mAppAppsList.data.size)
            for (info in mAppAppsList.data) {
                writer.println(
